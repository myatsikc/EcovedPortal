...
- **Роутер:** `192.168.1.254` (проброс портов 80 и 443 на порт 3000 сервера).
- **Домен:** `zelenavorona.ru`.

### Стек инфраструктуры
1. **Docker Desktop:** Для запуска контейнеров (Backend, Frontend).
2. **Docker Compose:** Один `docker-compose.yml` — для dev и prod.
   - `frontend:3000` — порт наружу (проброшен роутером с 80/443)
