...
## Общие инструкции
- **Правила работы с файлами (Обязательно):** 1) Перед любым редактированием **обязательно** используй `read_file` для загрузки актуального содержимого файла. 2) Для замены используй только точный фрагмент текста (`old_string`), скопированный из результата `read_file`. Это гарантирует, что ты редактируешь именно тот файл, который открыл. 3) После успешного редактирования **обязательно** перечитай файл через `read_file`, чтобы убедиться, что изменения применены корректно и не затронули другие файлы.
- **Процесс разработки:** Бизнес-требования -> Утверждение -> Технические спецификации -> Генерация кода -> Тестирование -> Коммит/Деплой.
