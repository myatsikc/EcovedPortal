...
4. Не дублируй одно и тоже требование в разных формах.
5. Не добавляй в документы пустые разделы. Если в раздел нечего писать - вообще пусть его не будет.
5. **Именование:** Используйте единый префикс для раздела (например, `news-`, `volunteers-`, `auth-`).
